...
// Sunny yellow gradient with warm solar colors
#define COLOR_SUN1  "\033[38;2;255;255;0m"     // Pure Yellow
#define COLOR_SUN2  "\033[38;2;255;215;0m"     // Gold
#define COLOR_SUN3  "\033[38;2;255;193;37m"    // Golden Yellow
#define COLOR_SUN4  "\033[38;2;255;165;0m"     // Orange
#define COLOR_SUN5  "\033[38;2;255;140;0m"     // Dark Orange
#define COLOR_SUN6  "\033[38;2;255;127;80m"    // Coral
#define COLOR_SUN7  "\033[38;2;255;99;71m"     // Tomato
#define COLOR_SUN8  "\033[38;2;255;69;0m"      // Red Orange
...
static const char* sun_colors[] = {
    COLOR_SUN1, COLOR_SUN2, COLOR_SUN3, COLOR_SUN4,
    COLOR_SUN5, COLOR_SUN6, COLOR_SUN7, COLOR_SUN8
};

const char* get_sun_color(int index) {
    return sun_colors[index & 7]; // Use bitwise AND for faster modulo
}

void print_sun_header() {
    printf("\033[2J\033[H"); // Clear screen
    
    // ASCII art header with gradient - SUN
    static const char* header_lines[] = {
        ...
        " ╚══════╝ ╚═════╝ ╚═╝  ╚═══╝",
        "",
        "☀️ SYSTEM INFO FETCHER ☀️",
        "  Run 'qfetch-setting' to configure"
    };
    
    for (int i = 0; i < 9; i++) {
        printf("%s%s%s%s\n", 
               COLOR_BOLD, get_sun_color(i), 
...
void print_info_line(const char* label, const char* value, int color_index) {
    printf("%s%s%-12s%s%s%s%s\n", 
           COLOR_BOLD, get_sun_color(color_index), label, 
...
void print_color_palette() {
    printf("%s☀️ Color Palette: ", COLOR_WHITE);
    for (int i = 0; i < 8; i++) {
        printf("%s██", get_sun_color(i));
    }
    printf("%s\n\n", COLOR_RESET);
}
